...
\begin{metainfo}
  \name{
    \lang{en}{...}
    \lang{de}{A01: unbestimmte Integrale}
  }
  \begin{description} 
...
\end{metainfo}
\begin{content}
\lang{de}{\title{A01: unbestimmte Integrale}}
\lang{en}{\title{A01: indefinite integrals}}
\begin{block}[annotation]
	Im Ticket-System: \href{https://team.mumie.net/issues/24074}{Ticket 24074}
\end{block}
\begin{block}[annotation]
    Kopie: hm4mint/T107_Integralrechnung/training 3 
...
\randomquestionpool{3}{5}
	...
	\begin{question}%1
		\lang{de}{
			\text{Gegeben sei die Funktion $f(x) = \var{f_1}$. \\
		      Bestimmen Sie für $x>0$ eine Stammfunktion $F(x)$ von $f(x)$:}}
		 \lang{en}{\text{
      Suppose the function $f(x) = \var{f_1}$ is given. \\
      Determine an antiderivative $F(x)$ of $f(x)$ for $x>0$:
     }}
		\type{input.function}
	...
	\begin{question}%2
	 ...
	 \lang{en}{\text{
      Suppose the function $g(x) = \var{f_7}$, $x>0$ is given. \\
    ...
    \type{input.generic}
      ...
      \lang{de}{\text{Es ist Zufall.}}
       \lang{en}{\text{It is a coincidence.}}
	    \solution{false}
	   \end{choice}

	   \begin{choice}
      \lang{de}{\text{Es kann passieren, dass durch Rechenfehler die gleiche Stammfunktion entsteht.}}
       \lang{en}{\text{It can happen that we get the same antiderivatives due to a calculation error .}}
	    \solution{false}
	   \end{choice}
 ...
 \lang{en}{\text{There are always exactly two different antiderivatives for a function.}}
      \solution{false}
       ...
       \lang{en}{
       \explanation[equalChoice(1???)]{In this case, the explanation we are looking for is not "coincidence".}
       \explanation[equalChoice(?1??)]{Calculation or rounding errors do not explain the observed equality here.}
       \explanation[equalChoice(??0?)]{Differentiating a constant yields $0$, so two antiderivatives can differ by a constant.}
       \explanation[equalChoice(???1)]{If there is an antiderivative, then there are infinitely many.}
       }       
\end{answer}
	\end{question}%3
