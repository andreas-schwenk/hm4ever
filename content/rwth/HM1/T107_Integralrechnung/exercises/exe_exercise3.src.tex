...
\begin{block}[annotation]
  Im Ticket-System: \href{http://team.mumie.net/issues/9603}{Ticket 9603}
\end{block}
...
\begin{incremental}[\initialsteps{1}]
    \step
    \lang{de}{Die Funktion $f(x)=e^{-x}\big(\cos(x)-\sin(x)\big)$ ist ein \emph{Produkt} von zwei Funktionen. }
    \lang{en}{The function $f(x)=e^{-x}\big(\cos(x)-\sin(x)\big)$ is the \emph{product} of two functions.}
   ...
   % $e^{-x}$ und $\cos(x)-\sin(x)$ führt nicht zum richtigen Ergebnis!
    ...
    \step
    \lang{de}{Nach der Produktregel für die Ableitung gilt:}
    ...
    \step
    \lang{de}{$F_3(x)$ ist die gesuchte Stammfunktion, denn nach der Produktregel gilt}
